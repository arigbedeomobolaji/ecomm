...
const PlaceOrder = (props) => {
    ...
    cart.itemsPrice = toPrice(cartItems.reduce((acc, current) => acc + current.price * current.qty, 0))
    cart.shippingPrice = cartItems.length > 5 ? 100 : cartItems.length > 0 ? 20 : 0 
    cart.taxPrice = toPrice(Number(cart.itemsPrice) * taxPercent)
    cart.orderTotal = toPrice(Number(cart.itemsPrice) + Number(cart.shippingPrice) + Number(cart.taxPrice)) 
