...
const OrderScreen = (props) => {
    ...
    const {userInfo: {name, email}} = userSignin
   ...
   <div className="row">
    <div className="col-lg">
     <div className="block">
      ...
      <p>
       <strong>Address:</strong> &nbsp;&nbsp; {order.shippingAddress.address}, {order.shippingAddress.city}
        ...
        {
            order.isDelivered ? 
            (<MessageBox variant={"success"}>Delivered at {order.deliveryDate}</MessageBox>) : 
     ...
     <div className="block">
      ...
      <p> <strong> Method: </strong> &nbsp;&nbsp; { order.paymentMethod }</p>
      {
            ...
            order.isPaid ? 
            (<MessageBox variant={"success"}>Paid on {moment(order.paidDate).format("dddd, MMMM Do YYYY, h:mm:ss a")}</MessageBox>) : 
    ...
    <div className="col-sm">
     <div className="block">
      ...
      <div className="row force">
       <div>Shipping</div>
       <div>₦{toPrice(order.shippingPrice).toFixed(2)}</div>
      </div>
      <div className="row force">
       <div>Tax</div>
      ...
      !order.isPaid && order.paymentMethod === "paystack" ? (
        <PaystackButton
          ...
          {...paystackComponentProps}></PaystackButton>
      ) :( 
        <button 
        type="button" 
        className="button button--primary button--block"
